    ...
    <header className="w-full py-3 px-4 sm:px-6">
      <nav className="flex items-center justify-between max-w-7xl mx-auto">
        ...
        <div className="hidden md:flex items-center space-x-3">
          <Link to={token ? "/app" : "/signup"} className={`px-3 py-2 rounded-md transition ${(token ? location.pathname === '/app' : location.pathname === '/signup') ? 'text-indigo-600' : 'text-slate-500 hover:text-slate-900'}`}>Home</Link>
      ...
      {isMobileMenuOpen && (
        <div className="md:hidden mt-4 pb-4 border-t border-gray-200">
          <div className="flex flex-col space-y-2 pt-4">
            <Link 
              to={token ? "/app" : "/signup"} 
              className={`px-3 py-2 rounded-md transition ${(token ? location.pathname === '/app' : location.pathname === '/signup') ? 'text-indigo-600 bg-indigo-50' : 'text-slate-500 hover:text-slate-900 hover:bg-gray-50'}`}
            ...
            <Link 
              to="/about" 
              className={`px-3 py-2 rounded-md transition ${location.pathname === '/about' ? 'text-indigo-600 bg-indigo-50' : 'text-slate-500 hover:text-slate-900 hover:bg-gray-50'}`}
              onClick={() => setIsMobileMenuOpen(false)}
            >
              About
            </Link>
